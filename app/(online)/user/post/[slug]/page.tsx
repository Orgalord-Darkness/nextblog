...
export default async function EditPostPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (!post || post.authorId !== Number(session.userId)) notFound()
    ...
    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
            <div className="max-w-2xl mx-auto px-6 py-12">
                ...
                <form action={editPost} className="flex flex-col gap-6" >
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                            Image {post.imageId ? "(remplacer l'image actuelle)" : "(optionnel)"}
                        </label>
                        {post.imageId && (
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">
                                Une image est déjà associée à ce post. Sélectionne un nouveau fichier pour la remplacer.
                            </p>
                        )}
                        <input
                            type="file"
                    ...
                    <div className="flex gap-3">
                        ...
                        <a
                            href="/user/posts"
                            className="px-6 py-2 border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 font-semibold rounded-lg hover:opacity-80 transition-opacity"
                        >
                            Annuler
                        </a>
                    </div>
